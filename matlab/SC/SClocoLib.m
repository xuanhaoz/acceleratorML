...
%	CM data structure.
	...
	function setupLOCOmodel(SC,varargin)
		...
		% Modification by fzhang %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		% add magnet offset to ideal lattice parameters so it can be used as a fitting parameter in loco
		%
		% offsetOrds = SCgetOrds(SC.IDEALRING,'^CF|^CD|^B1|^BDS|^Q|^S');		% only magnet ords, need to include BPM/COR if needed
		% for ord = offsetOrds
		% 	if ~isfield(RINGdata.Lattice{ord},'MagnetOffset')
		% 		RINGdata.Lattice{ord}.MagnetOffset = [0 0 0];
		% 		SC.IDEALRING{ord}.MagnetOffset = [0 0 0];
		% 	end
		% end

		% apply current CM setpoints to ideal model
		%
		% IdealRingWithCM = SC;
		% IdealRingWithCM.RING = SC.IDEALRING;
		% for nDim = 1:2
		% 	for ord = SC.ORD.CM{nDim}
		% 		% CMsp = SCgetCMSetPoints(SC,SC.ORD.CM{nDim},nDim);
		% 		% IdealRingWithCM = SCsetCMs2SetPoints(IdealRingWithCM,SC.ORD.CM{nDim},CMsp,nDim);
		% 		CMsp = SCgetCMSetPoints(SC,ord,nDim);
		% 		% IdealRingWithCM = SCsetCMs2SetPoints(IdealRingWithCM,ord,CMsp,nDim);
		% 		IdealRingWithCM.RING{ord}.KickAngle(nDim) = CMsp;
		% 	end
		% end
		% RINGdata.Lattice = IdealRingWithCM.RING;
		% End modifications %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% 		LocoMeasData.BPMSTD = Err;
		...
		for nFam = 1:length(varargin)
			...
			for ord = varargin{nFam}{1}
				...
				switch varargin{nFam}{2}
					...
					% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
					% modifications by F.Zhang, adding support for magnet offset as fitting parameter
					case 'offsetX'
						FitParameters.Params{nGroup,1}(nElem).FieldName = 'MagnetOffset';
					case 'offsetY'
						FitParameters.Params{nGroup,1}(nElem).FieldName = 'MagnetOffset';
				...
				% Modification by fzhang %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
				% Added support for magnet offset as fitting parameter
				% Added if statement
				%
				if strcmp(FitParameters.Params{nGroup,1}(nElem).FieldName,'MagnetOffset')
					switch varargin{nFam}{2}
						case 'offsetX'
							FitParameters.Values(nGroup,1)   = RINGdata.Lattice{ord}.(FitParameters.Params{nGroup}(nElem).FieldName)(1);
							FitParameters.IdealValues(nGroup,1) = SC.IDEALRING{ ord}.(FitParameters.Params{nGroup}(nElem).FieldName)(1);
							FitParameters.OrigValues(nGroup,1)  = RING0{ord}.(        FitParameters.Params{nGroup}(nElem).FieldName)(1);
						case 'offsetY'
							...
							FitParameters.OrigValues(nGroup,1)  = RING0{ord}.(        FitParameters.Params{nGroup}(nElem).FieldName)(2);
						otherwise
							error('Unsupported magnet offset type.')
					end
					% FitParameters.Values(nGroup,1)      = 0;
					% FitParameters.IdealValues(nGroup,1) = 0; 
					% FitParameters.OrigValues(nGroup,1)  = 0;
					% end modifications
				% End modifications %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
				else
					...
					% Modification by fzhang %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
					%
					% FitParameters.Values(nGroup,1)      = SC.RING{ord}.(FitParameters.Params{nGroup}(nElem).SCFieldName)(2);
					% End modifications %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	function applyLatticeCorrection(SC,FitParameters,varargin)
		...
		for nGroup = 1:length(FitParameters.Params)
			...
			for nElem = 1:length(FitParameters.Params{nGroup})
				...
				switch field
					case 'SetPointB'
						currentSetpoint = SC.RING{ord}.SetPointB(2);
					case 'SetPointA'
						currentSetpoint = SC.RING{ord}.SetPointA(2);
				end
				% original with scale factor
				% setpoint = FitParameters.OrigValues(nGroup) + p.Results.scaleFactor*(FitParameters.IdealValues(nGroup)-FitParameters.Values(nGroup));
				% according to lecture notes J. Safranek
				if FitParameters.IdealValues(nGroup) == 0 | FitParameters.OrigValues(nGroup) == 0
					correctionFactor = 1;
				else
					correctionFactor = (FitParameters.OrigValues(nGroup)/FitParameters.IdealValues(nGroup));
				end
				setpoint = FitParameters.OrigValues(nGroup) + p.Results.scaleFactor*(FitParameters.IdealValues(nGroup)-FitParameters.Values(nGroup))*correctionFactor;
	...
	function applyOrbitCorrection(SC,varargin)
		...
		[CUR,ERROR] = SCfeedbackRun(SC,par.Minv,'target',0,'maxsteps',1000,'BPMords',par.BPMords,'CMords',par.CMords,'verbose',par.verbose);
	...
	% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	% Plot status of the correction chain
	function plotEssentials(SC,Init,BPMData,CMData)
		...
		% DISTURBED LATTICE % % % % % % % % % % % % % % % % % % % % % % % %
		[ld,REAL.Tune,REAL.chromaticity] = atlinopt(Init.SC.RING,DP,REFPTS);
		REAL.beta                        = reshape([ld.beta],[2,nREF]);
		REAL.dispersion                  = reshape([ld.Dispersion],4,nREF);
		REAL.dispersion                  = REAL.dispersion([1 3],:);
		...
		% IDEAL LATTICE % % % % % % % % % % % % % % % % % % % % % % % % % %
		% [ld,Ideal.Tune,Ideal.chromaticity] = atlinopt(IDEAL.Lattice,1e-3,REFPTS);
		[ld,Ideal.Tune,Ideal.chromaticity] = atlinopt(SC.IDEALRING,DP,REFPTS);
		Ideal.beta                         = reshape([ld.beta],[2,nREF]);
		Ideal.dispersion                   = reshape([ld.Dispersion],4,nREF);
		Ideal.dispersion                   = Ideal.dispersion([1 3],:);

		% CORRECTED LATTICE % % % % % % % % % % % % % % % % % % % % % % % %
		[ld,CorrModel.Tune,CorrModel.chromaticity] = atlinopt(SC.RING,DP,REFPTS);
		CorrModel.beta                             = reshape([ld.beta],[2,nREF]);
		CorrModel.dispersion                       = reshape([ld.Dispersion],4,nREF);
		CorrModel.dispersion                       = CorrModel.dispersion([1 3],:);
		...
		quadOrds = SCgetOrds(SC.RING,'CF|CD|^Q');
		skewOrds = SC.ORD.SkewQuad;
		bendOrds = SCgetOrds(SC.RING,'B1|BDS');
		...
		% CorrModel.quad = [];
		for ord=quadOrds
			real = Init.SC.RING{ord}.PolynomB(2);
			corr = SC.RING{ord}.PolynomB(2);
			quadDelta(end+1) = (corr - real)/real;
		end
		for ord=skewOrds
			real = Init.SC.RING{ord}.PolynomA(2);
			corr = SC.RING{ord}.PolynomA(2);
			skewDelta(end+1) = (corr - real)/real;
		end
		for ord=bendOrds
			real = Init.SC.RING{ord}.PolynomB(2);
			corr = SC.RING{ord}.PolynomB(2);
			bendDelta(end+1) = (corr - real)/real;
		end
		subplot(2,3,3)
		scatter(quadOrds,100*quadDelta,6,'red','filled');hold on
		scatter(skewOrds,100*skewDelta,6,'green','filled');hold on
		scatter(bendOrds,100*bendDelta,6,'blue','filled');
		xlabel('Fit parameter');ylabel('$\Delta k_1$ [m$^{-1}$]');title('Change in Fit Parameter')
		% legend({sprintf('Normal quads rms: %.3f $m^{-1}$',sqrt(mean(abs(quadDelta))).^2),...
		% 		sprintf('Skew quads rms: %.3f $m^{-1}$',sqrt(mean(abs(skewDelta))).^2),...
		% 		sprintf('Dipole quds rms: %.3f $m^{-1}$',sqrt(mean(abs(bendDelta))).^2)})
		xlabel('Fit parameter');ylabel('$\Delta k_1$ [$\%$]');title('Change in Fit Parameter')
		legend({sprintf('Normal quads rms: %.1f \\%%',100*sqrt(mean(abs(quadDelta).^2))),...
				sprintf('Skew quads rms: %.1f \\%%',100*sqrt(mean(abs(skewDelta).^2))),...
				sprintf('Dipole quds rms: %.1f \\%%',100*sqrt(mean(abs(bendDelta).^2)))})
		%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		% CM strength %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		CMords = SCgetOrds(SC.RING,'COR');
		HCM = [];
		VCM = [];

		for ord=CMords
			HCM(end+1) = SC.RING{ord}.SetPointB;
			VCM(end+1) = SC.RING{ord}.SetPointA;
		end
		subplot(2,3,6)
		scatter(1:length(CMords),1E6*HCM,6,'red','filled');hold on
		scatter(1:length(CMords),1E6*VCM,6,'blue','filled');
		xlabel('Corrector magnet');ylabel('Kick angle [$\mu$rad]');title('Corrector Strengths')
		legend({sprintf('HCM rms: %.2d $\\mu$rad',1E6*sqrt(mean(abs(HCM).^2))),...
				sprintf('VCM rms: %.2d $\\mu$rad',1E6*sqrt(mean(abs(VCM).^2)))},'Interpreter','Latex')
